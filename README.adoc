...
* 'GPG_PRIVATE_KEY' : ASCII armored GPG private key block (optional)
* 'GPG_PUBLIC_KEY' : also optional
...
Spin up for testing inside container only
 sudo docker run -e CITIZEN_DATABASE_TYPE=sqlite -e CITIZEN_STORAGE=file -e CITIZEN_STORAGE_PATH=/citizen -v ./providers:/citizen -d 5966789db52a
